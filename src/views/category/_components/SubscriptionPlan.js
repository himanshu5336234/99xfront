...
export default function CategorySubscriptionPlan(props) {
    ...
    let [BuyBtnText, setBuyBtnText] = useState("Sign up")

    let { plans, minHeight, minHeightKeyFeatures, maxWidthKeyFeatures, title, subtitle, planUnit } = props;
                                ...
                                <div className="key-features">
                                    <ul style={{ minHeight: minHeightKeyFeatures || 'auto' }}>
                                        {MSelectedPlan.key_features.map((feature, index) => {
                                            ...
                                            if (boldRegexCheck) feature.replace(boldRegex, `<b>${boldRegexCheck[0]}</b>`)

                                            const highLightRegex = /\*\*(.+)\*\*/
                                            const highLightRegexCheck = feature.match(highLightRegex)
                                            if (highLightRegexCheck) feature.replace(highLightRegex, `<span class="highlight">${highLightRegexCheck[0]}</span>`)
                                ...
                                <div className="custom-tab-body">
                                    <div className="sub-table">
                                        <W3Carousel
                                            ...
                                            nav={false}
                                        >
                                            <div className="item item-labels" style={{minWidth:'250px'}}>
                                                    ...
                                                    <div className="item">
                                                        <div className="sub-card fluid">
                                                            ...
                                                            <div className="sub-heading">
                                                                <h3>{plan.title}</h3>
                                                                {plan.highlight && <b>{plan.highlight}</b>}
                                                                <p>
                                                                    {plan.description}
                                                                </p>
                                                            ...
                                                            <div className="key-features">
                                                                <ul style={{ minHeight: minHeightKeyFeatures || 'auto', maxWidth: maxWidthKeyFeatures || 'auto' }}>
                                                                    {plan.key_features.map((feature, index) => {

                                                                        // Order of Checking is Important
                                                                        const highLightRegex = /\*\*(.+)\*\*/
                                                                        const highLightRegexCheck = feature.match(highLightRegex)
                                                                        if (highLightRegexCheck) feature = feature.replace(highLightRegex, `<span class="highlight">${highLightRegexCheck[1]}</span>`)

                                                                        const boldRegex = /\*(.+)\*/
                                                                        const boldRegexCheck = feature.match(boldRegex)
                                                                        if (boldRegexCheck) feature = feature.replace(boldRegex, `<b>${boldRegexCheck[1]}</b>`)
                                                            ...
                                                            <div className="sub-features">
                                                                <ul style={{ minHeight: minHeight || '300px' }}>
                                                                    {Object.keys(plan.features).map((f, i) => {
                                                                        ...
                                                                        // if(plan.feature_empty && plan.feature_empty.indexOf(i) != -1){
                                                                        //     return (
                                                                        //         <>
                                                                        //         <li>
                                                                        //             {plan.features[f] ?
